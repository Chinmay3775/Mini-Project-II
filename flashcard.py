...
def normalize_text(text):
    # Remove IPA pronunciation (anything between slashes, e.g., /mÊŠmËˆbaÉª/)
    text = re.sub(r'/[^/]+/', '', text)
    
    # Remove pronunciation in brackets, e.g. [ËˆmumbÉ™i]
    text = re.sub(r'\[[^\]]+\]', '', text)
    
    # Remove language tags like 'Marathi: MumbaÄ«,' or 'Hindi: ...'
    text = re.sub(r'\b\w+:\s*[^,;()]+[,;)]?', '', text)
    
    # Remove special characters like â“˜ or others
